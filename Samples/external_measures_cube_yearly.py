...
external_factors_yearly = external_factors.copy()
external_factors_yearly['Year'] = [str(date)[:4] for date in list(external_factors_yearly.index)]
external_factors_yearly = external_factors_yearly.groupby('Year').mean()
for series_code in europe_series_codes:
    ...
    for year in list(external_factors_yearly.index):
        coordinates = ("Europe",year, series_code)
        cellset[coordinates] = float(external_factors_yearly.loc[year][series_code])
